...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, us_im=None, us_mask=None, coil_map=None):
...
class Unpatchify(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer(x)
        _, _, C = x.shape
        x = rearrange(x, 'b (h w) (p1 p2 c)-> b (h p1) (w p2) c', h = 128, w =128, p1=self.dim_scale, p2=self.dim_scale, c=C//(self.dim_scale**2))
...
class DataConsistency(nn.Module):
    def __init__(self, num_of_feat_maps, patchify, patch_size=2):
        ...
        self.patch_embed = PatchEmbed(patch_size=patch_size, in_chans=2, embed_dim=num_of_feat_maps, norm_layer=nn.LayerNorm)
...
class SwinTransformer_Unrolled(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=2, num_classes=2,
                 embed_dim=96, depths=[2, 2, 2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(
                img_size//patch_size, img_size//patch_size), dim_scale=patch_size, dim=embed_dim)
